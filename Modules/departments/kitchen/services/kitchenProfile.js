...
const kitchenDB = require("../../../../Models/kitchen/userSchema");
const saltRounds = 12;
...
const createUserSer = async (req) => {
  return new Promise(async (resolve, reject) => {
    try {
      let { kitchenName, firstName, lastName, phoneNumber, email, password } =
        req.body;
      ...
      let user = await kitchenDB.findOne({ phoneno: phoneNumber });
      if (user) {
        return resolve({
          status: 409,
        ...
        const savedUser = await kitchenDB.create(newUser);
        if (!savedUser) {
          return resolve({
            status: 500,
            message:
...
const otpVerficationSer = async (req) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let status = await otpVerfication(req, kitchenDB);
      if (status) {
        let user = await kitchenDB.findOneAndUpdate(
          {
...
const loginUserSer = async (req) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // }
      let user = await kitchenDB.findOne({
      ...
      return resolve({
        status: 200,
        message: "Successfully loggedIn as Kitchen",
        data: {
          firstName: user?.ownerName?.firstName,
          token: user.token,
        },
      });
    } catch (err) {
...
const logoutUserSer = async (req) => {
  return new Promise(async (resolve, reject) => {
    try {
      const kitchenId = mongoose.Types.ObjectId(req.loggedInDetails.userid);
      const { phoneNumber, email } = req.body;
      await kitchenDB.findOneAndUpdate(
        {
          $or: [{ phoneno: phoneNumber }, { email: email }, { _id: kitchenId }],
        },
        { token: null }
      );
      return resolve({
...
const sendOtpSer = async (req) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const user = await kitchenDB.findOne({
      ...
      if (!user) {
        ...
        msg91Service(user.name.firstName, mobileNo, otpCode);
...
const forgotPasswordSer = (req) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const user = await kitchenDB.findOneAndUpdate(
        {
          $or: [{ phoneno: phoneNumber }, { email: email }],
          otpCode: otpCode,
